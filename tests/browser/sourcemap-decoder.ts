...
export class SourceMapDecoder {
  ...
  private async getSourceMap(sourceMapUrl: string): Promise<SourceMapConsumer | null> {
    ...
    try {
      ...
      if (!response.ok) {
        console.warn(`Failed to fetch sourcemap: ${response.status} ${response.statusText}`);
        return null;
      }
      ...
      console.warn(`Error loading sourcemap from ${sourceMapUrl}:`, error.message);
      return null;
    }
  }
   ...
   */
  async decodeStackTraceLine(line: string, baseUrl: string = 'https://localhost:8080'): Promise<string> {
    // Match patterns like: "at https://localhost:8080/app.js:18423:13"
    // or "https://localhost:8080/app.js:18423:13"
    const stackTraceRegex = /(?:at\s+)?([https:\/\/[a-zA-Z0-9\-\.]+(?::\d{1,4})?[a-zA-Z0-9\-\/\.]*\.js):(\d+):(\d+)/g;
      ...
      try {
        ...
        if (consumer) {
          ...
          if (originalPosition.source && originalPosition.line) {
            ...
            const originalLocation = `${cleanSource}:${originalPosition.line}:${originalPosition.column || 0}`;
            decodedLine = decodedLine.replace(fullMatch, originalLocation);
          }
        }
      } catch (error) {
        console.warn(`Error decoding stack trace line "${fullMatch}":`, error.message);
      }
    }
   ...
   */
  async decodeStackTrace(stackTrace: string, baseUrl: string = 'https://localhost:8080', sourceMapData?: any): Promise<DecodedStackTrace> {
    ...
    if (sourceMapData) {
      ...
      for (const line of lines) {
        if (line.trim()) {
          const decodedLine = await this.decodeStackTraceLineWithConsumer(line.trim(), consumer);
      ...
      for (const line of lines) {
        if (line.trim()) {
          const decodedLine = await this.decodeStackTraceLine(line.trim(), baseUrl);
          decodedLines.push(decodedLine);
        }
      }
    }
   ...
   */
  private async decodeStackTraceLineWithConsumer(line: string, consumer: SourceMapConsumer): Promise<string> {
    const stackTraceRegex = /(?:at\s+)?([https:\/\/[a-zA-Z0-9\-\.]+(?::\d{1,4})?[a-zA-Z0-9\-\/\.]*\.js):(\d+):(\d+)/g;
