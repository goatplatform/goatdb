...
with both local storage and remote peers. When remote changes are received, they
are merged back into the in-memory Item. This [design](/docs/architecture)
provides immediate local updates while ensuring eventual consistency across all
peers.
...
rules to automatically prevent invalid data from being persisted or
