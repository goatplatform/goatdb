...
```typescript
create(path: string, schema: S, data: `Partial<`SchemaDataType`<S>>`): [ManagedItem](./classes/manageditem)<S, `Schema`>
```
...
```typescript
item(pathComps: string[]): [ManagedItem](./classes/manageditem)<S, `Schema`>
```
...
```typescript
query(config: `Omit<`QueryConfig`<`IS`, `OS`, `CTX`>, unknown>`): [Query](./classes/query)<`IS`, `OS`, `CTX`>
```
