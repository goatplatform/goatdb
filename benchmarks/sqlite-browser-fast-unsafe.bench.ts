...
import { getSQLiteWorkerManager, createSQLiteConfig, configureSQLiteUnsafe } from './sqlite-worker.ts';
...
async function populateDatabase(
  ...
  const countResult = await manager.exec(dbPath, 'SELECT COUNT(*) as count FROM test_items');
...
export default function setup(): void {
  BENCHMARK('SQLite Fast-Unsafe', 'Create instance', async (ctx) => {
    ...
    const config = configureSQLiteUnsafe(createSQLiteConfig(`temp_bench_sqlite_${uniqueId()}.db`));
  ...
  BENCHMARK('SQLite Fast-Unsafe', 'Open database (100k items)', {
    ...
    const manager = await getSQLiteWorkerManager();
    const config = configureSQLiteUnsafe(createSQLiteConfig('temp_bench_sqlite_100k.db'));
  ...
  BENCHMARK('SQLite Fast-Unsafe', 'Read 100k items', {
    ...
    assert(res.resultRows?.length === 100000, 'Database should have 100000 items');
  ...
  BENCHMARK('SQLite Fast-Unsafe', 'Create single item', async (ctx) => {
    ...
    ctx.start();
    await manager.exec(config.dbPath,
      'INSERT INTO test_items (id, title, count, tags) VALUES (?, ?, ?, ?)',
  ...
  BENCHMARK('SQLite Fast-Unsafe', 'Read item by ID', async (ctx) => {
    ...
    ctx.start();
    // Now read the item
    const readItem = await manager.exec(config.dbPath,
      'SELECT * FROM test_items WHERE id = ?',
      ['foo']
    );
    ctx.end();

    assert(readItem.resultRows?.[0]?.title === 'Test read item', 'Item title should match');
  ...
  BENCHMARK('SQLite Fast-Unsafe', 'Update item', async (ctx) => {
    ...
    // Verify updates
    const item = await manager.exec(config.dbPath,
      'SELECT * FROM test_items WHERE id = ?',
      [itemId]
    );
    assert(item.resultRows?.[0]?.title === 'Updated title', 'Item title should be updated');
  ...
  BENCHMARK('SQLite Fast-Unsafe', 'Bulk read 100 items', async (ctx) => {
    ...
    for (let i = 0; i < 100; i++) {
      ...
      assert(item.resultRows?.[0]?.title === `Item ${i}`, 'Item title should match');
    }
    ctx.end();
  ...
  BENCHMARK('SQLite Fast-Unsafe', 'Simple query', async (ctx) => {
    ...
    const results = await manager.exec(config.dbPath,
  ...
  BENCHMARK('SQLite Fast-Unsafe', 'Count operation', async (ctx) => {
    ...
    const countResult = await manager.exec(config.dbPath,
  ...
  BENCHMARK('SQLite Fast-Unsafe', 'Keys operation', async (ctx) => {
    ...
    ctx.start();
    // Test keys operation
    const keysResult = await manager.exec(config.dbPath,
